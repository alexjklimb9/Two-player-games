...
const C={bg:'#0b0f14',bg2:'#151b23',line:'rgba(235,240,245,.075)',line2:'rgba(235,240,245,.14)',top:'#4fb8d8',bottom:'#e06f8f',board:'#d7dde5',boardDark:'#2b333d',danger:'#d97757',white:'#f8fafc'};
...
function gap(){return Math.max(108,canvas.height*.18)}

function makePlatforms(){const arr=[];for(let i=0;i<26;i++){arr.push({x:canvas.width/2,y:baseY()-i*gap(),w:canvas.width*.78,h:14,tilt:0,tiltV:0,pulse:0,flashSide:null,flash:0})}return arr}
function makePlayer(side){return{side,x:side==='top'?canvas.width*.34:canvas.width*.66,y:baseY()-34,vx:0,vy:-280,r:21,on:0,grounded:false,lastGrounded:false,airTime:0,wobble:Math.random()*6.28,blink:Math.random()*4,color:side==='top'?C.top:C.bottom}}
...
function spark(x,y,color,n=10){for(let i=0;i<n;i++){const a=Math.random()*6.28,s=40+Math.random()*160;game.sparks.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:1,size:1.5+Math.random()*3.8,color})}}
function endGame(msg){if(!game.running)return;game.running=false;clearInput();game.shake=12;spark(canvas.width/2,game.camY+canvas.height*.5,C.board,26);resultTitle.textContent='Game Over';resultText.textContent=msg;finalStats.innerHTML=`Height Reached: ${game.score}<br>Launch Chain: ${game.combo}<br>Survival Time: ${Math.floor(game.timer)}s`;endOverlay.classList.remove('hidden')}
...
function slamSeesaw(lander,pf,landingSpeed){const side=lander.x<pf.x?'left':'right';const sideSign=side==='left'?-1:1;const strength=clamp(landingSpeed/520,.65,1.65);pf.tiltV+=-sideSign*.46*strength;pf.tilt+=-sideSign*.08*strength;pf.pulse=1;pf.flashSide=side;pf.flash=1;game.shake=Math.max(game.shake,clamp(landingSpeed*.014,4,12));spark(lander.x,pf.y-12,lander.color,12);
  let launched=false;
  game.players.forEach(other=>{if(other===lander)return;if(other.on!==lander.on)return;if(sameSide(lander,other,pf))return;const distanceFromEnd=clamp(Math.abs(other.x-pf.x)/(pf.w/2),.45,1.1);const impulse=clamp(410+landingSpeed*.72,540,940)*distanceFromEnd;other.vy=-impulse;other.grounded=false;other.airTime=0;other.y-=10;game.combo++;launched=true;spark(other.x,other.y,other.color,18)});
  if(!launched){lander.vy=-clamp(300+landingSpeed*.18,340,520);lander.y-=7;lander.grounded=false;lander.airTime=0}
}

function updatePlatforms(dt){game.platforms.forEach(pf=>{pf.tiltV+=(-pf.tilt)*2.25*dt;pf.tiltV*=Math.pow(.86,dt*60);pf.tilt+=pf.tiltV*dt*60;pf.tilt=clamp(pf.tilt,-.58,.58);pf.pulse=Math.max(0,pf.pulse-dt*3.4);pf.flash=Math.max(0,pf.flash-dt*4.2)});}
function updatePlayers(dt){game.players.forEach(p=>{p.lastGrounded=p.grounded;p.grounded=false;p.airTime+=dt;const d=dir(p);if(d)p.vx+=d*34*dt;p.vx*=Math.pow(.88,dt*60);p.x+=p.vx*dt*60;p.vy+=1080*dt;p.y+=p.vy*dt;p.wobble+=(3.1+Math.abs(p.vx)*.06+Math.abs(p.vy)*.0024)*dt;p.blink+=dt;
  const idx=nearestPlatform(p);const pf=game.platforms[idx];const surface=platformSurface(pf,p.x);const within=Math.abs(p.x-pf.x)<pf.w/2-22;const falling=p.vy>=0;const close=p.y>=surface-30&&p.y<=surface+38;
  if(falling&&within&&close){const landingSpeed=p.vy;p.y=surface;p.vy=0;p.grounded=true;p.on=idx;p.x=clamp(p.x,pf.x-pf.w/2+26,pf.x+pf.w/2-26);if(p.airTime>.08&&landingSpeed>130)slamSeesaw(p,pf,landingSpeed);p.airTime=0}
  if(!within&&p.grounded){p.grounded=false;p.vy=90}
  const ceilingY=pf.y-gap()+25;if(p.vy<0&&p.y-p.r<ceilingY&&idx<game.platforms.length-1){spark(p.x,p.y,C.danger,22);endGame(`${p.side==='top'?'Top':'Bottom'} player hit the spike ceiling.`)}
  if(p.y-game.camY>canvas.height+95)endGame(`${p.side==='top'?'Top':'Bottom'} player fell.`)
});
  const highestY=Math.min(game.players[0].y,game.players[1].y);game.camY+=(highestY-canvas.height*.48-game.camY)*.052;
  let h=0;game.players.forEach(p=>{h=Math.max(h,Math.max(0,Math.floor((baseY()-p.y)/gap())))});game.score=Math.max(game.score,h);
}
function updateSparks(dt){for(let i=game.sparks.length-1;i>=0;i--){const s=game.sparks[i];s.x+=s.vx*dt;s.y+=s.vy*dt;s.vy+=175*dt;s.life-=dt*2.15;if(s.life<=0)game.sparks.splice(i,1)}}
function update(dt){if(game.running){game.timer+=dt;updatePlatforms(dt);updatePlayers(dt);timeText.textContent=`${Math.floor(game.timer)}s`;stressText.textContent=`${Math.min(100,Math.floor(Math.max(...game.platforms.map(p=>Math.abs(p.tilt)))/.58*100))}%`;fallText.textContent=game.score;holdText.textContent=`${Math.max(1,game.combo).toFixed(0)}x`}updateSparks(dt);game.shake=Math.max(0,game.shake-dt*20)}
...
function drawSpikes(pf,i){if(i>=game.platforms.length-1)return;const y=sy(pf.y-gap()+25);if(y<-20||y>canvas.height+30)return;ctx.fillStyle='rgba(217,119,87,.82)';const count=9,span=pf.w*.82,start=pf.x-span/2;for(let k=0;k<count;k++){const x=start+k*(span/(count-1));ctx.beginPath();ctx.moveTo(x-9,y);ctx.lineTo(x+9,y);ctx.lineTo(x,y+18);ctx.closePath();ctx.fill()}}
function drawPlatform(pf,i){const y=sy(pf.y);if(y<-90||y>canvas.height+105)return;drawSpikes(pf,i);ctx.save();ctx.translate(pf.x,y);ctx.rotate(pf.tilt);ctx.shadowColor='rgba(0,0,0,.32)';ctx.shadowBlur=15+pf.pulse*10;ctx.shadowOffsetY=8;ctx.fillStyle=C.boardDark;rr(-pf.w/2,-7-pf.pulse*3,pf.w,14+pf.pulse*6,7);const danger=Math.min(1,Math.abs(pf.tilt)/.58);const gr=ctx.createLinearGradient(-pf.w/2,0,pf.w/2,0);gr.addColorStop(0,C.top);gr.addColorStop(.5,danger>.55?C.danger:C.board);gr.addColorStop(1,C.bottom);ctx.fillStyle=gr;rr(-pf.w/2,-2.5,pf.w,5,2.5);if(pf.flash>0){ctx.globalAlpha=pf.flash*.45;ctx.fillStyle=pf.flashSide==='left'?C.top:C.bottom;const x=pf.flashSide==='left'?-pf.w/2:0;rr(x,-9,pf.w/2,18,8);ctx.globalAlpha=1}ctx.shadowBlur=0;ctx.fillStyle='rgba(255,255,255,.7)';ctx.beginPath();ctx.arc(0,0,4,0,6.28);ctx.fill();ctx.restore();ctx.strokeStyle='rgba(203,213,225,.24)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(pf.x,y+7);ctx.lineTo(pf.x-16,y+56);ctx.lineTo(pf.x+16,y+56);ctx.closePath();ctx.stroke()}
