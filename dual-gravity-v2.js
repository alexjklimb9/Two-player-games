// Dual Gravity tutorial build: shared center line, spikes, enemies, wall puzzle, and one shared teammate launch pad.
...
const game={running:false,t:0,cameraX:0,doorOpen:false,input:{tl:false,tm:false,tr:false,bl:false,bm:false,br:false},prevInput:{tm:false,bm:false},top:{x:110,y:0,w:24,h:30,vx:0,vy:0,on:false,g:1,color:'#4fb8d8'},bottom:{x:110,y:0,w:24,h:30,vx:0,vy:0,on:false,g:-1,color:'#e06f8f'}};
...
function clearInput(){Object.keys(game.input).forEach(k=>game.input[k]=false);game.prevInput.tm=false;game.prevInput.bm=false}
function padBox(x,w,side){const y=seam();return{x,y:side==='top'?y-12:y,w,h:12}}
function launchPadBox(x,w){const y=seam();return{x,y:y-16,w,h:32}}
...
function makeLevel(){const y=seam();
  ...
  walls=[
    ...
    {x:1430,y:y-86,w:34,h:172},
    {x:2365,y:y-104,w:36,h:208},
    {x:2855,y:y-124,w:38,h:248},
    {x:3185,y:y-70,w:30,h:140}
  ];
  switches=[
  ...
  launchPads=[{x:1305,w:110,cool:0}];
  spikeButtons=[
  ...
  game.top.x=110;game.top.y=y-game.top.h;game.bottom.x=110;game.bottom.y=y;game.top.vx=game.top.vy=game.bottom.vx=game.bottom.vy=0;game.top.on=game.bottom.on=true;game.cameraX=0;game.doorOpen=false;game.prevInput.tm=false;game.prevInput.bm=false}
...
function launchStatus(){for(const p of launchPads){const pad=launchPadBox(p.x,p.w);const topReady=game.top.on&&rects(game.top,pad),bottomReady=game.bottom.on&&rects(game.bottom,pad);if(topReady&&bottomReady)return{pad:p,topReady,bottomReady}}return null}
function updateLaunchPads(dt,status,topJumpPressed,bottomJumpPressed){for(const p of launchPads)p.cool=Math.max(0,p.cool-dt);if(!status||status.pad.cool>0)return;if(topJumpPressed&&status.bottomReady){game.bottom.vy=-game.bottom.g*520;game.bottom.vx+=145;status.pad.cool=1.05}else if(bottomJumpPressed&&status.topReady){game.top.vy=-game.top.g*520;game.top.vx+=145;status.pad.cool=1.05}}
...
function update(dt){if(!game.running)return;game.t+=dt;const padBeforeMove=launchStatus(),topJumpPressed=game.input.tm&&!game.prevInput.tm,bottomJumpPressed=game.input.bm&&!game.prevInput.bm;movePlayer(game.top,dt,game.input.tl?1:0,game.input.tr?1:0,game.input.tm||game.input.tl&&game.input.tr);movePlayer(game.bottom,dt,game.input.bl?1:0,game.input.br?1:0,game.input.bm||game.input.bl&&game.input.br);updateLaunchPads(dt,padBeforeMove,topJumpPressed,bottomJumpPressed);updateSpikes(dt);for(const sw of switches)sw.active=rects(sw.side==='top'?game.top:game.bottom,sw);game.doorOpen=switches.every(s=>s.active);for(const e of enemies){if(!e.alive)continue;e.x+=e.vx*dt;if(e.x<2500||e.x>2740)e.vx*=-1;if(e.side==='top'&&rects(game.top,e))endGame('The top player was caught.');if(e.side==='bottom'&&rects(game.bottom,e))endGame('The bottom player was caught.')}if(game.doorOpen&&game.top.x>EXIT_X&&game.bottom.x>EXIT_X)endGame('Both players completed the first Dual Gravity training course.',true);game.cameraX=clamp(teamX()-canvas.width/2,0,CAMERA_MAX);game.prevInput.tm=game.input.tm;game.prevInput.bm=game.input.bm}
