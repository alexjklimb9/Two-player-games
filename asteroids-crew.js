...
function resetGame(){Object.assign(game,{running:true,timer:0,score:0,hits:0,shake:0,spawn:0,angle:-Math.PI/2,shipV:0,weapon:0,shield:0,rocks:[],shots:[],sparks:[]});clearInput();resetWeapons();makeStars();startOverlay.classList.add('hidden');endOverlay.classList.add('hidden')}
...
function spawnRock(){const side=Math.floor(Math.random()*4);let x,y;if(side===0){x=-40;y=Math.random()*canvas.height}else if(side===1){x=canvas.width+40;y=Math.random()*canvas.height}else if(side===2){x=Math.random()*canvas.width;y=-40}else{x=Math.random()*canvas.width;y=canvas.height+40}const dx=cx()-x,dy=cy()-y,a=Math.atan2(dy,dx)+(-.45+Math.random()*.9);const sp=48+Math.random()*42+game.timer*1.4;const r=18+Math.random()*22;game.rocks.push({x,y,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp,r,hp:Math.ceil(r/17),rot:Math.random()*6.28,spin:-1+Math.random()*2})}
...
function updateShip(dt){if(game.input.topLeft)game.shipV-=220*dt;if(game.input.topRight)game.shipV+=220*dt;game.shipV*=Math.pow(.92,dt*60);if(game.input.bottomLeft)game.angle-=3.1*dt;if(game.input.bottomRight)game.angle+=3.1*dt}
...
function setTouch(t,on){const top=t.clientY<canvas.height/2,left=t.clientX<canvas.width/2;if(top){if(left&&on&&!game.input.topLeft)selectWeapon(1);if(left)game.input.topLeft=on;else game.input.topRight=on}else{if(left)game.input.bottomLeft=on;else{game.input.bottomRight=on;if(on)fire()}}}
